...
pre_datas_trains=["file/pre_datas_train1.csv","file/pre_datas_train2.csv","file/pre_datas_train3.csv",
                 "file/pre_datas_trapre_datas_trainsin4.csv","file/pre_datas_train5.csv","file/pre_datas_train6.csv",
...
def pre_process():
    ...
    labels=normal_labels+xssed_labels
    ...
    if w== 'kfold':
        ...
        for train,test in folder.split(datas,labels):
            ...
            train = np.random.choice(train, size = len(train)//1, replace = False)
            test = np.random.choice(test, size = len(test)//1, replace = False)
            #print(train)
...
def dataTest(model_dir,test_generator,test_size,input_num,dims_num,batch_size):
    ...
    for batch,labels in test_generator:
        if len(labels)==batch_size:
            ...
            print("-----------------------")
            z = 1/(1 + np.exp(-result))
            print(result,"      ", labels,"     ",z,"     ",f"...")
            print("-----------------------")
            print("-----------------------")

            labels_pre.extend(result)

        else:
            batch=np.concatenate((batch,np.zeros((batch_size-len(labels),input_num,dims_num))))
            result = model.predict_on_batch(batch)[0:len(labels)]
            print("-----------------------")
            print("-----------------------")
            z = 1/(1 + np.exp(-result))
            print(result,"      ", labels,"     ", z,"     ",f"...")
